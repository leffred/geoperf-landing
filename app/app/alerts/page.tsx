    ...
    <Section py="md" tone="cream">
      <div className="flex items-baseline justify-between mb-4 flex-wrap gap-3">
        <div>
          ...
          <h1 className="font-serif text-3xl text-navy">{alertList.length} alerte{alertList.length > 1 ? "s" : ""}</h1>
      ...
      <div className="flex flex-wrap gap-2 mb-6 text-xs">
        <Link href={`/app/alerts${baseQS({ show: undefined })}`} className={`px-3 py-1.5 ${!show ? "bg-navy text-white" : "bg-white hover:bg-cream"}`}>Toutes</Link>
        <Link href={`/app/alerts${baseQS({ show: "unread" })}`} className={`px-3 py-1.5 ${show === "unread" ? "bg-navy text-white" : "bg-white hover:bg-cream"}`}>Non lues</Link>
        <span className="border-l border-navy/15 mx-1"></span>
        {(["high", "medium", "low"] as const).map(s => (
          <Link key={s} href={`/app/alerts${baseQS({ severity: severity === s ? undefined : s })}`} className={`px-3 py-1.5 ${severity === s ? "bg-navy text-white" : "bg-white hover:bg-cream"}`}>
            {SEV_LABEL[s]}
          </Link>
        ))}
        {(brand || severity || type || show) && (
          <Link href="/app/alerts" className="px-3 py-1.5 text-ink-muted hover:text-navy underline">
            Réinitialiser
          </Link>
        )}
      </div>
      ...
      {alertList.length === 0 ? (
        ...
        <div className="space-y-2">
          {alertList.map(a => (
            <article key={a.id} className={`border-l-2 p-4 ${SEV_STYLES[a.severity] || SEV_STYLES.low} ${a.is_read ? "opacity-70" : ""}`}>
              <div className="flex items-baseline justify-between gap-3 mb-1.5 flex-wrap">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="font-mono text-[10px] uppercase tracking-widest opacity-70">{TYPE_LABELS[a.alert_type] || a.alert_type}</span>
                  <span className="font-mono text-[10px] uppercase tracking-widest opacity-70">· {SEV_LABEL[a.severity]}</span>
                  {!a.is_read && <span className="font-mono text-[10px] px-1.5 py-0.5 bg-amber text-navy">NEW</span>}
                  {a.email_sent_at && <span className="font-mono text-[10px] opacity-60">📧 envoyée</span>}
                </div>
                <span className="font-mono text-[10px] opacity-60 whitespace-nowrap">{fmtDate(a.created_at)}</span>
              </div>
              <h3 className="font-serif text-base font-medium mb-1">
                <Link href={`/app/brands/${a.brand_id}`} className="hover:underline">
                  {brandsMap[a.brand_id] ?? "?"} — {a.title}
                </Link>
              </h3>
              <p className="text-sm leading-relaxed mb-2">{a.body}</p>
              <div className="flex items-center gap-3 text-xs">
                <Link href={`/app/brands/${a.brand_id}`} className="opacity-80 hover:opacity-100 underline">
                  Ouvrir la marque →
                </Link>
                {!a.is_read && (
                  <form action={markOneAlertRead}>
                    <input type="hidden" name="alert_id" value={a.id} />
                    <button type="submit" className="opacity-80 hover:opacity-100 underline">
                      Marquer lue
                    </button>
                  </form>
                )}
